...
const extractProductData = async (url,browser) => {
    try{
        ...
        //titulo
        productData['name'] = await page.$eval('.inside-page-hero h1', name=>name.innerText)
        //precio
        ...
        //info
        productData['info'] = await page.$eval(".event-post-item-row", info=>info.innerText)
        //descripción
        productData['description'] = await page.$eval('.entry-content', description=>description.innerText.slice(0,200) + '...')
        // //Datos del evento
        // productData['Datos del evento'] = await page.$eval('.post-content__info-event', data=>data.innerText.slice(0,200) + '...')
